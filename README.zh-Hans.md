...
| 工具 | 版本 | 用途 |
|------|------|------|
| [Node.js](https://nodejs.org/) | **≥ 22.13** | 仅 Web / 文档开发与前端构建 |
...
以上要求见根目录 [`package.json`](package.json)（`engines.node`、`packageManager`）。CI 使用 **Node 22** 与 **pnpm 11**。安装 Release 包的终端用户**不需要** Node.js。
...
见上方 **[版本与依赖](#-版本与依赖)**。简要来说：**Node.js ≥ 22.13**、**pnpm 11.x**；桌面端开发还需 **Rust（stable）**。
